...
@section('title', 'Generate Bill — Sagar Motors')
@section('page-title', 'Generate Bill')
@section('page-subtitle', 'Create a new service job card and bill')

@section('content')
<form method="POST" action="{{ route('bills.store') }}" id="bill-form" class="space-y-6">
    @csrf
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" id="customer_name" name="customer_name" value="{{ old('customer_name') }}" required placeholder="e.g. Ramesh Patel"
                ...
                <div>
                    ...
                    <input type="text" id="mobile" name="mobile" value="{{ old('mobile') }}" required maxlength="15" placeholder="e.g. 9876543210"
                           class="w-full bg-gray-50 border border-gray-300 rounded-lg px-4 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 transition-all">
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" id="car_number" name="car_number" value="{{ old('car_number') }}" required placeholder="e.g. GJ-03-AB-1234"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="car_name" name="car_name" value="{{ old('car_name') }}" required placeholder="e.g. Swift, Creta"
                               class="w-full bg-gray-50 border border-gray-300 rounded-lg px-4 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 transition-all">
                    </div>
                    <div>
                        <label for="car_model" class="block text-xs font-semibold text-gray-700 uppercase tracking-wider mb-1.5">Model / Year</label>
                        <input type="text" id="car_model" name="car_model" value="{{ old('car_model') }}" placeholder="e.g. 2022 VXi"
                               class="w-full bg-gray-50 border border-gray-300 rounded-lg px-4 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 transition-all">
                    </div>
                </div>
                <div>
                    ...
                    <select id="car_company" name="car_company" required
                        ...
                        @foreach($carCompanies as $company)
                            <option value="{{ $company->name }}" {{ old('car_company') == $company->name ? 'selected' : '' }}>{{ $company->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <select id="service_type" name="service_type" required
            ...
            <option value="denting" {{ old('service_type') == 'denting' ? 'selected' : '' }}>Denting Work</option>
            <option value="painting" {{ old('service_type') == 'painting' ? 'selected' : '' }}>Painting Work</option>
            <option value="general_service" {{ old('service_type') == 'general_service' ? 'selected' : '' }}>General Service & Repair</option>
        </select>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div class="overflow-x-auto">
            <table class="w-full" id="items-table">
                ...
                <tbody id="items-body">
                    <tr class="item-row border-b border-gray-100" data-row="0">
                        <td class="px-3 py-2.5 text-sm text-gray-400 row-number">1</td>
                        <td class="px-3 py-2.5">
                            <input type="text" name="items[0][part_name]" required
                                   ...
                                   placeholder="e.g. Front Bumper Denting & Painting">
                        </td>
                        <td class="px-3 py-2.5">
                        ...
                        <td class="px-3 py-2.5">
                            <input type="number" name="items[0][price]" value="0" min="0" step="0.01" required
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="space-y-4">
                <div class="flex items-center justify-between py-2 border-b border-gray-100">
                    ...
                    <span id="display-subtotal" class="text-sm font-bold text-gray-900">₹0.00</span>
                </div>
                <div>
                    ...
                    <input type="number" id="labor_cost" name="labor_cost" value="0" min="0" step="0.01"
                           class="w-full bg-gray-50 border border-gray-300 rounded-lg px-4 py-2.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 transition-all"
                ...
                <div class="flex items-center justify-between py-3 border-t-2 border-gray-200 mt-2">
                    ...
                    <span id="display-grand-total" class="text-2xl font-extrabold text-amber-600">₹0.00</span>
                </div>
            </div>
        </div>
